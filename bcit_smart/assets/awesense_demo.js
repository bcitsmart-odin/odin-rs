...
function createAwesenseSettingsWindow() {
    ...
    return ui.Window("Awesense Data Settings", AWESENSE_SETTINGS, "./asset/bcit_smart/button_svg.svg")(ui.LayerPanel(AWESENSE_SETTINGS, checkboxToggleShowPoints), ui.Panel("data sets", true, "awesense-settings-window")(...checkboxes), ui.Panel("legend", true, "awesense-legend-window")(legend));
}
/**
...
function awesensePointSelection() {
    ...
    if (sel && sel._type && SUPPORTED_ELEMENT_TYPES.includes(sel._type)) {
        ...
        const detailsWindow = ui.getWindow(GRID_ELEMENT_DETAILS);
...
function formatGeometry(geometry) {
    if ("Point" in geometry && Array.isArray(geometry.Point)) {
        ...
        const formattedCoords = geometry.LineString.map(([x, y]) => `[${x.toFixed(4)}, ${y.toFixed(4)}]`);
 ...
 */
function buildGridElementDetailsVisualization(pointName, window) {
    ...
    if (content) {
        ...
        if (gridElement) {
            ...
            const gridElementProperties = ui.Panel("grid-element-properties", true)(...propertiesHTML);
            ;
            ...
            ["Source", "Down", "Same Voltage", "Connected"].forEach(traceName => {
                ...
                traceButton.addEventListener("click", () => {
                    ...
                    ws.sendWsMessage("bcit_smart/awesense_demo", "trace_request", { trace_name: traceName, grid_element_id: gridElement.grid_element_id });
                });
                // Append the button to the container
                buttonContainer.appendChild(traceButton);
            });
            const tracePanel = ui.Panel("trace-panel", true)(buttonContainer);
...
function handleWsMessages(msgType, msg) {
    ...
    // console.log(msg);
    switch (msgType) {
        case "awesense_element_list":
            handleAwesenseElementList(msg);
            break;
        case "awesense_trace_response":
            handleAwesenseTraceResponse(msg);
            break;
        default:
            {
...
function handleAwesenseTraceResponse(response) {
        ...
        if (element.geometry && element.geometry.Point) {
            ...
            const pointEntity = new Cesium.Entity({
                ...
                billboard: icon ? {
                    ...
                    width: 32,
                    height: 32,
                    scaleByDistance: new Cesium.NearFarScalar(500.0, 1.0, 2000.0, 0.4)
                } : undefined,
                description: element.grid_element_id, // Tooltip text for the point
                name: element.grid_element_id, // Name of the entity
                ...
                label: {
                    ...
                    scaleByDistance: new Cesium.NearFarScalar(500.0, 1.0, // Full visibility
